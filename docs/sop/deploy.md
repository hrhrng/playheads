...
- 中文 voice mode 默认走 `VOICE_STT_PROVIDER=whisper` + `VOICE_STT_LANGUAGE=zh`，由 `WhisperBufferedSTT` 把浏览器传来的 16 kHz PCM 分段后交给 `@cf/openai/whisper-large-v3-turbo`。只有英文实时测试才切回 `VOICE_STT_PROVIDER=flux`。
