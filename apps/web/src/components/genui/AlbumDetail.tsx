...
export function AlbumDetail({
  ...
  return (
    <div className="animate-genui-card-in">
      ...
      <div
        className="flex items-start gap-3 cursor-pointer rounded-2xl hover:bg-chip p-1.5 -m-1.5 transition-colors"
      ...
      >
        ...
        <div className="flex-1 min-w-0 py-0.5">
          <p className="text-[13px] font-display font-medium text-ink leading-tight line-clamp-1">{title}</p>
          <p className="text-[11px] text-ink-3 mt-0.5">{subtitle}</p>
          <div className="flex items-center gap-2 mt-0.5">
            {year && <span className="text-[10px] text-ink-4">{year}</span>}
            {tracks.length > 0 && <span className="text-[10px] text-ink-4">{tracks.length} tracks</span>}
          </div>
        </div>

        {/* Chevron */}
        <svg className={`w-4 h-4 text-ink-3 mt-1 transition-transform shrink-0 ${expanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
      ...
      {/* Expanded tracklist */}
      {expanded && (
        <div className="mt-2 ml-[4.25rem] pl-3 space-y-0 animate-genui-slide-in border-l-2" style={{ borderLeftColor: 'rgb(var(--accent) / 0.5)' }}>
          {loadingTracks && (
