...
export function TrackCard({
  ...
  return (
    <div className="flex items-center gap-3 py-2 px-3 rounded-2xl hover:bg-chip transition-colors group animate-genui-slide-in">
      <div className="w-10 h-10 rounded-card overflow-hidden bg-chip shrink-0">
        {artworkUrl ? (
          <>
            {!imageLoaded && <div className="w-full h-full bg-chip-2 animate-pulse" />}
          ...
          <div className="w-full h-full bg-chip flex items-center justify-center animate-pulse">
            <svg className="w-4 h-4 text-ink-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 19V6l12-3v13" /></svg>
          </div>
        )}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-display font-medium text-ink truncate">{title}</p>
        <p className="text-[11px] text-ink-3 truncate">{artist}{album ? ` \u00b7 ${album}` : ''}</p>
      </div>
      {canPlay && (
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={handlePlay} className="w-7 h-7 rounded-full bg-ink text-page flex items-center justify-center hover:scale-110 transition-transform" title="Play">
            <svg className="w-3 h-3 ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
          </button>
          <button onClick={handleQueue} className="w-7 h-7 rounded-full bg-chip-2 text-ink-2 flex items-center justify-center hover:scale-110 transition-transform" title="Add to queue">
