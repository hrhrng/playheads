        ...
        {hasTitle && (
          ...
          >
            <div className="flex-1 min-w-0">
              {props.title && <h3 className="text-sm font-display font-semibold text-ink">{props.title}</h3>}
              {props.subtitle && <p className="text-[11px] text-ink-3 mt-0.5">{props.subtitle}</p>}
              {collapsed && props.collapsedHint && (
                <p className="text-[10px] text-ink-4 mt-1">{props.collapsedHint}</p>
              )}
            </div>
            <svg className={`w-4 h-4 text-ink-3 shrink-0 transition-transform duration-200 ${collapsed ? '-rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
