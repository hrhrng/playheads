...
export const RecordPlayer = ({ currentTrack, isPaused, togglePlay, playbackTime, onSeek }: RecordPlayerProps) => {
    const [isDragging, setIsDragging] = useState(false);
    const [dragValue, setDragValue] = useState(0);

    const formatArtwork = (url: string | undefined, size = 600) => {
    ...
    const trackName = attr.name || attr.title || 'Unknown Track';
    const artistName = attr.artistName || 'Unknown Artist';
    const artworkUrl = formatArtwork(attr.artwork?.url || currentTrack.artwork?.url || currentTrack.artworkURL);
                ...
                <div className="w-full max-w-sm mt-4 flex items-center gap-3">
                    ...
                    <div className="flex-1">
                        <Slider
                            ...
                            onChange={(val) => {
                                setIsDragging(true);
                                setDragValue(typeof val === 'number' ? val : val[0]);
                            }}
                            onAfterChange={(val) => {
                                const finalVal = typeof val === 'number' ? val : val[0];
