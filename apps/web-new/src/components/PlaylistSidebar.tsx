...
export const PlaylistSidebar = ({
    ...
    const formatArtwork = (url: string | undefined, size = 100) => {
    ...
    const queue = (propQueue && propQueue.length > 0) ? propQueue.map(item => ({
        ...
        title: (item as any).title || (item as any).attributes?.name || item.name || 'Unknown Title',
        artist: (item as any).artistName || (item as any).attributes?.artistName || item.artist || 'Unknown Artist',
        cover: formatArtwork((item as any).artworkURL || (item as any).artwork?.url || (item as any).attributes?.artwork?.url || item.artwork_url)
    })) : [];
